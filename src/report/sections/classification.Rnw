...
<<>>=
  ...
  d <- data.frame(
    seqid = names(aln),
    n.over = lapply(aln, length) %>% unlist,
    n.ortho = lapply(aln, score) %>% lapply(function(x) sum(x > 40)) %>% unlist
  )

  # Proportion of genes whose search intervals include at least 1 significant hit
  lapply(aln, score) %>% lapply(function(x) any(x > 40)) %>% unlist %>% sum %>% divide_by(N)

  # Proportion of genes that link to given number of orthologs
  d$n.ortho %>% factor %>% summary
  
  lapply(aln, score) %>% lapply(length) %>% unlist %>% factor %>% summary
@
\begin{figure}[!ht]
    \centering
<<>>=
hist(lapply(aln, score) %>% unlist %>% log2)
@
    \caption{Scores of all local alignments against target genes in search intervals}
    \label{fig:old-score}
\end{figure}
