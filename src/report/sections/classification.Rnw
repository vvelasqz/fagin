...
if(file.exists(query.cache)){
  ...
  que <- LoadQuery(config, l_seqinfo)
  save(que, file=query.cache)
}
...
labels <- determineLabels(que, results)
@
...
\begin{figure}[!ht]
  \centering
<<cache=TRUE, message=FALSE>>=
lapply(results, function(x) x$sflags) %>%
  melt %>%
  dplyr::mutate(
    size = width(que$aa[seqid]),
    orph = ifelse(seqid %in% que$orphans, 'orphan', 'non-orphan')
  ) %>%
  ...
  ggplot(aes(x=flag, y=size)) +
    ...
    facet_wrap(species~orph, scale='free_y') +
    scale_y_continuous(
      trans='log2',
...
\begin{figure}[!ht]
  \centering
<<fig.width=6, fig.height=4, message=FALSE, cache=TRUE>>=
# TODO: Kill this fugly monster
m <- lapply(results, function(x) x$features$mRNA) %>%
    melt %>%
    dplyr::rename(species=L1) %>%
    count(query, species) %>%
    dplyr::rename(N.mrna=n)
# All the following sin just to get the zeros in
z <- split(m, m$species) %>%
    lapply(
      function(x){
        a <- setdiff(names(que$aa), x$query)
        data.frame(
          query=a,
          species=rep(x$species[1], length(a)),
          N.mrna=rep(0, length(a))
        )
      }
    ) %>%
    do.call(what=rbind) %>%
    set_rownames(NULL)
# Now bind the ugly sisters together
rbind(m, z) %>%
  count(species, N.mrna) %>%
  ggplot(aes(x=N.mrna, y=n)) +
    geom_point() +
    geom_line(aes(group=species), alpha=0.05) +
...
merge(
  ...
  results %>%
    lapply(function(x) x$fo.mrna$orphan2target$query) %>%
    lapply(unique) %>%
    lapply(length) %>%
    melt %>%
...
results %>%
  ...
  dplyr::mutate(
    orphan = ifelse(seqid %in% que$orphans, 'orphan', 'non-orphan'),
    group = sub('.queries', '', group)
  ) %>%
  dplyr::count(group, species, orphan) %>%
  dcast(group + orphan ~ species, mean, value.var='n') %>%
  dplyr::arrange(orphan, group) %>%
  xtable(
    display=c('s', 's', 's', 'd', 'd'), 
    caption=
...
results %>%
  ...
  melt %>%
  dplyr::mutate(
    orphan = ifelse(value %in% que$orphans, 'orphan', 'non-orphan')
  ) %>%
  dplyr::rename(species=L1) %>%
  dplyr::count(species, orphan) %>%
  dcast(orphan ~ species, mean, value.var='n') %>%
  xtable(
    caption=
...
m <- results %>% 
  lapply(function(x) x$prot2prot.scores[c('query', 'score')]) %>%
  melt(id.vars=c('query', 'score')) %>%
  dplyr::rename(species=L1) %>%
  dplyr::mutate(orphan = query %in% que$orphans)
