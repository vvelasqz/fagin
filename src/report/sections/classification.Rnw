...
plotOne <- function(q="AT4G18000.1", target=NULL, k=10000L){
    if(is.null(target)){
    ...
    bindAndPlot <- function(chrname, d) {
        ...
        td$y[td$side == 'query'  & td$group == 'out']   <- -1
        td$y[td$side == 'target' & td$group == 'syn']   <-  1 
        td$y[td$side == 'target' & td$group == 'out']   <-  1 
        td$y[td$side == 'query'  & td$group == 'query'] <- -1.1
        td$y[td$side == 'target' & td$group == 'si']    <-  1.1
        td$y <- td$y + seq(from=-0.025, to=0.025, length.out=nrow(td))
...
# plotOne(q=query$orphans[100], k=10000)
#
# plotOne(q=query$orphans[1], k=1000)
#
# plotOne(q=query$orphans[2])
#
# plotOne(q=query$orphans[3])
#
#
# plotOne(q=query$orphans[200], k=10000)
#
