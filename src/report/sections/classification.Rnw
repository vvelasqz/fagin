...
ggplot() +
  ...
  geom_point(
    ...
    data=subset(m, score > config$prot2prot_minscore & orphan) %>%
  ...
  geom_hline(yintercept=config$prot2prot_minscore, color='red') +
...
@
  \caption{\
    ...
    are not homologous. \textbf{In red:} the orphan genes with scores above
    \Sexpr{config$prot2prot_minscore} against genic targets within their search
    intervals.
  }
\end{figure}
...
<<results='asis', cache=TRUE>>=
m %>%
  subset(score > config$prot2prot_minscore) %>%
  dplyr::mutate(orphan = ifelse(orphan, 'orphan', 'non-orphan') %>% factor) %>%
  count(species, orphan) %>%
  xtable(
    caption=sprintf('The number of query orphans and non-orphans with
    significant ($score > %d$) similarity to annotated genes in each target
    genome.', config$prot2prot_minscore)
  )
@
% <<>>=
% m <- data.frame(
...
\subsection{Detect protein hits to spliced ORFs}
...
m <- results %>% 
  lapply(function(x) x$prot2transorf[c('query', 'score')]) %>%
  melt(id.vars=c('query', 'score')) %>%
  dplyr::rename(species=L1)
ggplot(m) +
  geom_violin(
  ...
  geom_point(
    mapping=aes(x=species, y=score),
    data=subset(m, score > config$prot2transorf_minscore),
    position=position_jitter(width=0.8),
    alpha=0.3
  ) +
  geom_hline(yintercept=config$prot2transorf_minscore, color='red') +
  coord_flip() +
  xlab('Raw alignment score (BLOSUM80)') +
  scale_y_continuous(
    trans='log2',
...
ggplot(m) +
  ...
  geom_point(
    mapping=aes(x=species, y=score),
    data=subset(m, score > config$prot2allorf_minscore),
    position=position_jitter(width=0.8),
    alpha=0.3
  ) +
  geom_hline(yintercept=config$prot2allorf_minscore, color='red') +
  coord_flip() +
  xlab('Raw alignment score (BLOSUM80)') +
  scale_y_continuous(
    trans='log2',
...
ggplot(m) +
  ...
  geom_point(
    mapping=aes(x=species, y=score),
    data=subset(m, score > config$gene2si_minscore),
  ...
  geom_hline(yintercept=config$gene2si_minscore, color='red') +
  coord_flip() +
  facet_grid(group~.)
...
\caption{\
  ...
  orphans with scores above \Sexpr{config$gene2si_minscore}.
}
\end{figure}
...
m %>%
  ...
  dplyr::mutate(signif=score > config$gene2si_minscore) %>%
  ggplot +
      geom_point(
        aes(
          x=qwidth,
...
\caption{\
  ...
  considered significant if it is above a certain threshold
  (\Sexpr{config$gene2si_minscore}).
}
\end{figure}
