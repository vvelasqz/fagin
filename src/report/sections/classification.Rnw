...
if(file.exists(rdat)){
  ...
  save(query, target, nstring, sflags, origins, bittbl, fo.cds, fo.mrna, file=rdat)
}
@
...
if(file.exists(alnrdat)){
  ...
  alnsum <- mutate(aln$scores, ortholog = score > 40) %>%
    group_by(query) %>%
    summarize(
      n.over = length(query),
      n.orth = sum(ortholog)
...
<<>>=
  ...
  sum(d$n.orth > 0) / n.total

  # summary of the number of orthologs found
  d$n.orth %>% factor %>% summary

  # summary of the number of overlapping genes found
  d$n.over %>% factor %>% summary
@
\begin{figure}[!ht]
    \centering
<<>>=
hist(lapply(old.aln, score) %>% unlist %>% log2)
@
