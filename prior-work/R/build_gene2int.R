Build_Gene2Int <- function(syn, gff, strata){
    # $ OUTPUT EXAMPLE 
    #    qseqid   chr tarid qstart   qend     tseqid tstart  tend   pident strand ps               name tlen qlen
    # ATMG00030  Chr1     1  11918  12241 scaffold_1      0   333 0.742120      -  9      Spermatophyta  334  324
    # ATMG00150  Chr1    43  35782  36132 scaffold_1  20480 20729 0.840000      - 19         Camelineae  250  351
    # ATMG00513  Chr1    20 140724 142998 scaffold_1  11684 11714 1.000000      -  1 cellular_organisms   31 2275
    # ATMG00516  Chr1    17 143219 147048 scaffold_1   8243  8272 1.000000      -  1 cellular_organisms   30 3830
    # ATMG00516  Chr1    16 143219 147048 scaffold_1   5682  6232 0.879342      -  1 cellular_organisms  551 3830
    # ATMG00516  Chr1    18 143219 147048 scaffold_1   9128  9364 0.819742      -  1 cellular_organisms  237 3830
    # Where
    ...
    #  * name - phylostratum name
    #  * ps - phylostratum level (where 1 is cellular_organisms)
    ...
    stopifnot(setequal(names(qsyn), names(genes)))
    ...
    ori <- gene2int$qseqid
    gene2int <- merge(gene2int, strata, by.x='qseqid', by.y='locus', all=TRUE)

    # warn if any genes are lost
    if(any(is.na(gene2int$ps))){
        msg <- "The following genes have no strata info:"
        seqids <- paste(unique(gene2int$qseqid[which(is.na(gene2int$name))]), collapse=" ") 
        warning(paste(msg, seqids))
    }
