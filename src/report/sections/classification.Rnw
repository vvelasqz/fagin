...
if(file.exists(rdat)){
  ...
  scaflen <- read.table('~/src/git/cadmium/input/stat/scaffold-lengths.tab', header=TRUE, stringsAsFactors=FALSE)
  qinfo <- PrepareSeqinfo(scaflen, 'Arabidopsis_thaliana')
  tinfo <- PrepareSeqinfo(scaflen, 'Arabidopsis_lyrata')
  ...
  # TODO: Fix the out-of-range bugs, may need to look into Synder
  target <- LoadTarget(
