...
getBlast <- function(querySeq, targetSeq, evalue=1e-3, num_threads=8, outfilename=paste0(querySeq, "_Blast_", targetSeq, ".txt")){
  makeblastdb(c('-in', paste0(targetSeq, '.faa'),'-dbtype', 'prot', '-out', paste0(targetSeq, '_blastDatabase'), '-title', paste0(targetSeq, '_blastDatabase')))
  blastp(c('-query', paste0(querySeq, '.faa'), '-db', paste0(targetSeq, '_blastDatabase'), '-num_threads', num_threads, '-outfmt', '"6 qseqid sseqid evalue"', '-evalue', evalue, '-out', outfilename))
  
}

filteringBlast <- function(blastfile,  threshold=1e-10, outfilename){
  blastfile1 <- read.table(blastfile)
  write(paste0(unique((subset(blastfile1, V3 > threshold))$V1)), outfilename)
}

#the input_query file has the name of the fasta file of the query. The input file has the name of the fasta files of the targets  
...
while (length(line <- readLines(input, n = 1, warn = FALSE)) > 0) {
  #making blast
  target <- data.frame(strsplit(line, ".", fixed = TRUE))[1,]
