...
\subsection{Synteny successes: the good, the bad, and the ugly}
...
@
  \caption{We nail these simplest case}
  \label{fig:good-1}
\end{figure}
...
@
  \caption{Duplications and overlaps with extensions, no problems.}
  \label{fig:good-2}
\end{figure}
...
@
  \caption{Weird duplication event is handled nicely.}
  \label{fig:good-2}
\end{figure}
...
@
  \caption{Case 1: Lack of strand awareness results in the search interval
    being placed on the wrong side of a block (\textbf{top}).}
  \label{fig:case-1}
\end{figure}
...
@
\caption{Case 2: Clean flag==2 case extends further than needed. It would be
  better to snap to the end of the nearest adjacent block.}
  \label{fig:case-1}
\end{figure}
...
@
\caption{Case x: Clean flag==3 case extends further than needed. Again,
  snapping would be cleaner.}
  \label{fig:case-1}
\end{figure}
...
@
\caption{Case 3: (\textbf{search interval on right}) incorrectly asserts an
...
@
\caption{Case 4: This looks like it should be a clean flag==0. Why the double
  search interval?}
  \label{fig:case-4}
\end{figure}
...
@
  \caption{Case 2a: slightly overlapping regions split badly}
  \label{fig:case-2a}
\end{figure}
...
@
  \caption{Case 2b: slightly overlapping(?) regions split badly}
  \label{fig:case-2a}
\end{figure}
...
@
  \caption{Case 3: Homologous intervals that overlap in both target and query.
    Perhaps these should be merged, but there is something weird going on at
    the sequence level that causes this. Maybe they should be merged with a
    flag.}
  \label{fig:case-3}
\end{figure}
...
@
  \caption{Case 3b: Again homologous intervals overlapping on both sides
    disrupt an otherwise clean flag==0 case.}
  \label{fig:case-2a}
\end{figure}
...
@
  \caption{Case 4: Large singleton insertion. The search interval should extend past the insertion, clipping to the nearest block. Likewise on the target side, it should clip somewhere ...}
  \label{fig:case-4}
\end{figure}
...
plotOne(q='AT3G25719.1', target, k=5000)
@
  \caption{Case 5: I have no idea why there is a second SI}
  \label{fig:case-4}
\end{figure}
...
@
  \caption{Case 6a: Extreme distance}
  \label{fig:case-6a}
\end{figure}
...
\begin{figure}[!ht]
  \centering
<<cache=TRUE>>=
plotOne(q='AT3G42148.1', target, k=10000)
@
  \caption{Case 6b: A Hardboiled Wonderland. There are a bunch of genes in this
...
@
  \caption{Case 6c: THIS IS SPARTA}
  \label{fig:case-6c}
\end{figure}
...
\begin{figure}[!ht]
  \centering
<<cache=TRUE>>=
plotOne(q='AT55G56795.1', target, k=2000)
@
  \caption{Case 7: Multiple non-adjacent insertions.}
  \label{fig:case-6c}
\end{figure}
