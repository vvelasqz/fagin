...
# OverlappingOrphans <- function(gint){
#     require(plyr)
#     orphan.level <- max(as.numeric(levels(gint$ps)))
# 
#     is.orphan = with(gint, ps == orphan.level)
#     has.match = with(gint, !is.na(tchr)) # gene overlaps synteny block
#     orphans <- subset(gint, is.orphan & has.match) 
#     simple.overlap <- ddply(orphans, 'qseqid', summarize,
#                             samechr=length(unique(tchr)) == 1)
# }
